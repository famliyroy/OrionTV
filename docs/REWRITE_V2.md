...
> 分支：`rewrite/v2`　当前版本：**v2.0.5**　后端：自部署 MoonTVPlus `https://tv.668664.xyz`（`225.1.0` / kvrocks）
>
...
├── index.tsx            首页：轮播 + 快捷入口（搜索/源站寻片）+ 6 模块
│                        （v2.0.2 起"继续观看"并入 me.tsx，见 §2.1 末尾）
...
- 壳导航只剩 **首页 / 我的 / 设置** 三个页签 —— 搜索从壳里撤掉，
  入口只保留首页快捷入口的「搜索」与「源站寻片」两块等分瓦片；
- **继续观看从首页并入「我的」**：「我的」页三个页签 收藏 / 观看记录 / 账号，
  观看记录每张卡片右上角自带删除按钮（`DELETE /api/playrecords?key=…`，单条删除），
  首页不再展示，设置里的"显示继续观看"开关随之移除
  （`continueWatchingEnabled` 落盘键保留，只为兼容旧数据）。
...
### 9.5 v2.0.4 - v2.0.5 交互体验与手势增强纪要

针对用户反馈的交互痛点完成系统性修复：

1. **资源列表响应式宽度自适应（消灭右侧大留白）**：
   - 原先卡片宽度固定，导致手机屏幕仅能容纳 2 列并留下 100dp+ 空白；
   - 重构 `computeMetrics`，改为根据当前屏幕可用宽度及计算出的列数精确反求卡片宽度，实现手机/平板/TV 各端严丝合缝填满无多余留白。
2. **播放器主流手势操作体系（对齐 B站 / 爱奇艺 / 腾讯视频规范）**：
   - **屏幕左半侧垂直滑动**：线性调节屏幕亮度，配备纯黑半透明平滑遮罩层与居中 HUD 图标/百分比提示（支持 10%~100% 亮度调节，零原生权限、跨端稳定）。
   - **屏幕右半侧垂直滑动**：线性调节播放器音量，内置 900ms 自动淡出的居中音量 HUD 指示条；底层 `ExpoAvAdapter` 扩展 `setVolume` 接口并持久化当前音量状态。
   - **轻触屏幕**：快速切换控制条显隐（弥补原底层视图吃掉触摸事件导致无法唤出/隐藏控制栏的问题）。
   - **双击屏幕**：在 300ms 窗口内触发暂停 / 播放切换。
   - **面板互斥保护**：侧栏面板展开时手势自动让渡，防止误触。
3. **弹幕渲染性能深度优化（告别密集弹幕卡顿）**：
   - 提取 `DanmakuLine` 独立组件并使用 `React.memo` 封装，基于 `entry.index` 与可见状态做精准局部渲染；
   - 将内部 `DanmakuText` 直接内联化，剔除冗余组件层与 hooks 运行开销；
   - 弹幕引擎轮询帧率由 16ms 调整为 100ms 降载（动画仍由 Reanimated UI 线程以 60fps 平滑驱动），显著降低 JS 线程负荷。

### 9.6 v2.0.6 播放沉浸感、控制条防抖与多分辨率自适应网格（2026-09-24）

针对实际多端设备（手机、各类尺寸平板）使用中反馈的痛点进行系统性攻关：

1. **播放器控制条闪烁与自动弹出彻底根治**：
   - **根因定位**：原有 `useEffect` 强依赖 `playerState.status`，在底层视频分片（HLS chunk）缓冲时频繁产生毫秒级 `buffering` 状态波动，每次波动切回 `playing` 时均强行触发 `bumpControls()`，造成“控制条刚自动隐藏又瞬间自动弹出”的恶性死循环。
   - **状态机重构**：引入 `prevStatusRef` 状态跃迁检测。仅当由 `paused` 恢复为 `playing` 时触发一次 4 秒自动隐藏倒计时，暂停状态保持常驻；彻底剥离播放中的微小缓冲抖动对控制栏的干扰，控制条展现完全由真实交互与关键跃迁驱动。
2. **系统虚拟触控条沉浸式全屏隐藏（Android NavigationBar）**：
   - 引入 `expo-navigation-bar`，在全屏播放器装载时通过 `NavigationBar.setVisibilityAsync('hidden')` 与 `NavigationBar.setBehaviorAsync('overlay-swipe')` 隐藏 Android 底部/侧边虚拟触控条与顶栏状态栏，提供 100% 纯净全屏沉浸视效；
   - 退出播放器（unmount）时自动恢复系统导航条与状态栏，避免影响系统全局交互。
3. **平板与多分辨率卡片自适应黄金网格（Elastic Grid）**：
   - **平板断点识别对齐官方规范**：`isWideScreen` 判定全面对齐 Android 官方 `sw600dp` 标准（短边 ≥ 600dp 或长边 ≥ 960dp），确保 7~11 寸平板启动时即能正确匹配平板壳。
   - **精准扣除导航侧栏宽度**：在 `computeMetrics` 中扣除平板常驻侧栏（72dp）与 TV 侧栏（88dp*scale），消除可用宽度计算偏大造成的卡片右侧溢出与布局错位。
   - **多端弹性列数与卡片尺寸算法**：
     - TV 端：沙发 10-foot 远距基准（约 180dp*scale，5~6 列）；
     - 平板端：卡片黄金舒适宽度基准 132dp，随屏幕分辨率（700~1400dp）平滑扩展为 4~8 列，杜绝平板上出现 2~3 个庞大海报卡片的比例崩坏问题；
